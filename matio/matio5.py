...
class MatRead5:
    ...
    def find_opaque_dtype(self, arr, subsystem, path=()):
        ...
        if arr.dtype == object:
            ...
            for idx in np.ndindex(arr.shape):
                ...
                if cell_item.dtype == OPAQUE_DTYPE:
                    ...
                    self.find_opaque_dtype(cell_item, subsystem, path + (idx,))
            ...
            for idx in np.ndindex(arr.shape):
                for name in arr.dtype.names:
                    ...
                    if field_val.dtype == OPAQUE_DTYPE:
                        ...
                        arr[idx][name] = subsystem.read_mcos_object(
                            metadata, type_system
                        )
                    else:
                        self.find_opaque_dtype(field_val, subsystem, path + (idx, name))
    ...
    def read_matfile(
        ...
        for var, data in matfile_dict.items():
            if not isinstance(data, np.ndarray):
                continue
            matfile_dict[var] = self.find_opaque_dtype(data, subsystem)
