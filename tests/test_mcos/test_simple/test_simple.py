...
@pytest.mark.parametrize("expected_array, file_name, var_name", [
        (
...
def test_parse_string_in_cell(expected_array, file_name, var_name):
    ...
    assert matdict[var_name][0,0]["_Props"].shape == expected_array[0,0]["_Props"].shape
    assert matdict[var_name][0,0]["_Props"].dtype == expected_array[0,0]["_Props"].dtype

    # Each property
    for prop, val in expected_array[0,0]["_Props"][0,0].items():
        np.testing.assert_array_equal(matdict[var_name][0,0]['_Props'][0,0][prop], val)
