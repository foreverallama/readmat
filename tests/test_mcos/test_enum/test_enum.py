        ...
        (
            {
                ...
                "_BuiltinClassName": None,
                "_Tag": "EnumerationInstance",
                "_ValueNames": np.array(['enum1', 'enum3', 'enum5', 'enum2', 'enum4', 'enum6']).reshape(2, 3),
                "_Values": np.array([
                    {
                        "val": np.array([1]).reshape(1, 1),
                    },
...
def test_enum_instance(expected_array, file_name, var_name):
    ...
    # Value Names
    np.testing.assert_array_equal(matdict[var_name]["_ValueNames"], expected_array["_ValueNames"])
                                ...
                                [
                                    {
                                        ...
                                        "_BuiltinClassName": None,
                                        "_Tag": "EnumerationInstance",
                                        "_ValueNames": np.array(['enum2']).reshape(1, 1),
                                        "_Values": np.array([
                                            {
                                                "val": np.array([2]).reshape(1, 1),
                                            }
                                ...
                                [
                                    {
                                        ...
                                        "_BuiltinClassName": None,
                                        "_Tag": "EnumerationInstance",
                                        "_ValueNames": np.array(['enum3']).reshape(1, 1),
                                        "_Values": np.array([
                                            {
                                                "val": np.array([3]).reshape(1, 1),
                                            }
...
def test_enum_inside_object(expected_array, file_name, var_name):
    ...
    for prop, val in expected_props.items():
        if prop == "cellProp":
            ...
            assert nested_actual_dict["_BuiltinClassName"] == nested_expected_dict["_BuiltinClassName"]
            
            # Value Names
            np.testing.assert_array_equal(nested_actual_dict["_ValueNames"], nested_expected_dict["_ValueNames"])

            # Values
            assert nested_actual_dict["_Values"].shape == nested_expected_dict["_Values"].shape
            assert nested_actual_dict["_Values"].dtype == nested_expected_dict["_Values"].dtype
